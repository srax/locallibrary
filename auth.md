...
user.last_name = 'Citizen'
user.save()
For more information, see Using a custom user model when starting a project (Django docs).
...
Click the Add button (next to Group) to create a new Group; enter the Name "Library Members" for the group.Admin site - add group
...
Click the Add button next to Users to open the Add user dialog box.Admin site - add user pt1
...
In the Groups section, select Library Member group from the list of Available groups, and then press the right-arrow between the boxes to move it into the Chosen groups box.Admin site - add user to group
